...
# eclipse-logo.txt has fastfetch's $1 to $9 in front of each change of shade, and every line starts
# with its shade. these are the nine shades from dim to pale, in true colour, and the forms of the
# logo the image installs
{ lib }:
let
  colours = [
    "38;2;115;84;38"
  ...
  lines = lib.splitString "\n" (lib.removeSuffix "\n" (builtins.readFile ./eclipse-logo.txt));
  eachLine = f: lib.concatMapStrings (line: f line + "\n") lines;
in
{
  file = ./eclipse-logo.txt;
  inherit colours;
