...
//! how rift-flash writes on macOS and Windows. rift-flash writes the partition table itself,
...
use librift::disk::run::{self, Drive, copy_to};
use librift::disk::{ALIGN, Bus, Disk, Partition, SECTOR, Table, needed, plan, size, write_gpt};

use crate::drive::check_file;
use crate::image::{Image, Reader};
use crate::{Listed, Request};

/// What rift-flash needs from a system to write a drive onto one of its disks.
pub trait System {
...
/// `rift-flash write`: shows the image and the disk, has its serial or name typed back and writes
...
pub fn write(
    ...
    if request.models.is_some() {
        return Err(
            "rift-flash cannot copy models onto a drive on this system, since the drive makes persist at its first boot. Nothing was written."
                .into(),
        );
    }
    ...
    say(format!("{} holds Rift {version}.", request.image.display()));
    ...
    written.map_err(|why| {
        format!(
            "{why}\nThe drive was not finished, and {shown} does not boot. Run rift-flash again to start over."
        )
    })?;
    say(format!(
        "{shown} is a Rift drive now, with version {version}. It makes persist when it first starts, with a passphrase you choose then."
    ));
    Ok(())
}
...
mod tests {
    ...
    use librift::disk::{GIB, GPT_BYTES, MIB, read_gpt};
    ...
    fn disk(name: &str, bus: Bus) -> Disk {
        Disk {
            ...
            serial: None,
            volumes: vec![librift::disk::Volume {
    ...
    fn a_stick_gets_the_image_s_slot_and_an_empty_slot_b() {
        ...
        assert_eq!(said[0], format!("{} holds Rift 0.1.0.", image.display()));
        ...
        assert!(
            said.last()
                .unwrap()
                .starts_with("/dev/disk4 is a Rift drive now")
        );
    ...
    fn list_shows_sticks_and_why_they_are_refused() {
        ...
        assert_eq!(
            ...
            [
                ...
                "It holds STICK.",
                "/dev/disk4 holds 16.0 GiB, and Rift needs 21.1 GiB.",
                "",
