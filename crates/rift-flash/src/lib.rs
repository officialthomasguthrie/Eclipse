//! rift-flash: writes Rift onto a stick or a USB disk from a system image. `list` shows the
...
//!
//! The command line and the app are both this library. The app lists the disks with [`disks`] and
//! checks the image with [`version`], and then starts the command line with the rights to write, so
//! the code that erases a disk runs the same way from both.
...
use librift::disk::Disk;

#[cfg(target_os = "linux")]
const USAGE: &str = "Usage: rift-flash list
       sudo rift-flash write [--exchange <size>] [--models <folder> | --first-boot] [--serial <serial>] <image> <disk>";

#[cfg(target_os = "linux")]
const HELP: &str = "Writes Rift onto a stick or a USB disk: the system in the image, an empty slot \
...
Everything on the disk is erased, so rift-flash shows the disk first and asks you to type its \
...
Commands:
  ...
  write              Erase a disk and write Rift onto it
...
#[cfg(target_os = "macos")]
const USAGE: &str = "Usage: rift-flash list
       sudo rift-flash write [--exchange <size>] [--serial <name>] <image> <disk>";

#[cfg(target_os = "macos")]
const HELP: &str = "Writes Rift onto a stick or a USB disk: the system in the image and an empty \
slot for the next update. The drive makes persist, encrypted with a passphrase you choose, when it \
first starts. Everything on the disk is erased, so rift-flash shows the disk first and asks you to \
...
const USAGE: &str = "Usage: rift-flash list
       rift-flash write [--exchange <size>] [--serial <serial>] <image> <disk>";
...
PhysicalDrive2, and rift-flash list shows the names. The image is a .raw or .raw.zst file. For a \
...
/// A stick or a USB disk that is plugged in, and why a drive cannot be written onto it now.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Listed {
    /// The disk.
    pub disk: Disk,
    /// The sentence that says why it is refused, when it is.
...
/// Runs rift-flash with `args`, the words after its name, and returns its exit code.
#[must_use]
pub fn cli(args: &[String]) -> ExitCode {
    match parse(args) {
        ...
        Ok(Command::Version) => {
            println!("rift-flash {}", librift::VERSION);
            ExitCode::SUCCESS
        }
...
fn run(command: &Command) -> Result<(), String> {
    match command {
        Command::List => {
            for line in lines(&disks()?) {
                println!("{line}");
            }
            Ok(())
        }
        Command::Write(request) => write(request),
        Command::Help | Command::Version => Ok(()),
    }
}

/// The sticks and USB disks `rift-flash list` shows, each with the sentence that says why a drive
/// cannot be written onto it now, when it cannot. It needs no root.
///
/// # Errors
///
/// When the system does not say what its disks are.
pub fn disks() -> Result<Vec<Listed>, String> {
    #[cfg(target_os = "linux")]
    return linux::listed();
    #[cfg(any(target_os = "macos", windows))]
    return Ok(direct::listed(&system()?));
    #[cfg(not(any(target_os = "linux", target_os = "macos", windows)))]
    return Err(UNSUPPORTED.into());
}

/// The version of Rift in the image at `path`, whose partition table is read and checked the way
/// `write` reads it.
///
/// # Errors
///
/// The sentence that says why the file is not an image rift-flash can write.
pub fn version(path: &Path) -> Result<String, String> {
    image::Image::open(path).map(|image| image.slot.version)
}

/// What `rift-flash list` prints: each disk, why it is refused when it is, and a blank line
/// between two.
fn lines(listed: &[Listed]) -> Vec<String> {
    let mut lines = Vec::new();
    for Listed { disk, refused } in listed {
        if !lines.is_empty() {
            lines.push(String::new());
        }
        lines.extend(disk.describe());
        lines.extend(refused.clone());
    }
    if lines.is_empty() {
...
#[cfg(target_os = "macos")]
fn system() -> Result<macos::Mac, String> {
    macos::Mac::query()
}

#[cfg(windows)]
fn system() -> Result<windows::Windows, String> {
...
#[cfg(any(target_os = "macos", windows))]
fn write(request: &Request) -> Result<(), String> {
    direct::write(request, &system()?, &mut ask, &mut |line| {
        println!("{line}");
    })
}

#[cfg(not(any(target_os = "linux", target_os = "macos", windows)))]
fn write(_: &Request) -> Result<(), String> {
    Err(UNSUPPORTED.into())
}

#[cfg(not(any(target_os = "linux", target_os = "macos", windows)))]
const UNSUPPORTED: &str = "rift-flash cannot write a drive on this system.";
...
mod tests {
    ...
    fn write_takes_an_image_a_disk_and_options() {
        assert_eq!(
            parsed(&["write", "rift.raw.zst", "/dev/sdb"]),
            Ok(Command::Write(Request {
                image: "rift.raw.zst".into(),
                target: "/dev/sdb".into(),
                exchange: None,
                models: None,
                first_boot: false,
        ...
        assert_eq!(
            parsed(&["write", "--first-boot", "rift.raw", "drive.img"]),
            Ok(Command::Write(Request {
                image: "rift.raw".into(),
    ...
    fn mistakes_are_refused_before_anything_is_looked_at() {
        ...
        assert!(parsed(&["write", "rift.raw"]).is_err());
