...
/// A whole disk opened for rift-flash alone. Its blocks are dropped from the page cache before it
...
/// `sudo rift-flash write`: shows the image and the disk, asks for the serial and the passphrase,
/// and writes the drive.
pub fn write(request: &Request) -> Result<(), String> {
    if !rustix::process::geteuid().is_root() {
        return Err(
            "Writing a drive erases a disk, so it needs root. Run sudo rift-flash write <image> <disk>."
                .into(),
        );
    }
    ...
    println!("{} holds Rift {version}.", plan.image.path.display());
    ...
    let unfinished = |why: String| {
        format!(
            "{why}\nThe drive was not finished, and {path} does not boot. Run rift-flash again to start over."
        )
    };
    if request.first_boot {
        write_without_persist(&plan, &mut |line| println!("{line}")).map_err(unfinished)?;
        println!(
            "{path} is a Rift drive now, with version {version}. It makes persist when it first starts, with a passphrase you choose then."
        );
        return Ok(());
    }
    ...
    println!(
        "{path} is a Rift drive now, with version {version}. Persist opens with the passphrase you chose."
    );
    Ok(())
}
