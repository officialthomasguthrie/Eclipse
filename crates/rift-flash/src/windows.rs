...
use librift::disk::{Disk, GET_DISK, read_get_disk};

use crate::direct::System;

/// The Windows computer rift-flash runs on.
pub struct Windows {
    administrator: bool,
    disks: Vec<Disk>,
}
...
impl System for Windows {
    ...
    fn rights(&self) -> Option<&'static str> {
        (!self.administrator).then_some(
            "Writing a drive erases a disk, so it needs an administrator. Open the terminal with Run as administrator and run rift-flash write again.",
        )
    }
