//! `eclipse clone`: a second drive, written onto a removable disk. Vault does the work as root, so
//! this runs `vault clone` in its place with the same arguments.

use std::os::unix::process::CommandExt;
use std::process::{Command, ExitCode};

const USAGE: &str = "Usage: sudo eclipse clone [--serial <serial>] <disk>";
