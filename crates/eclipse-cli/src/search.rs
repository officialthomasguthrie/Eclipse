//! `eclipse ai index` and `eclipse ai search`: search by meaning in home. The index is the owner's
...
use libeclipse::aura::{self, Client};
use libeclipse::search::{self, Hit, Index, Kind};

use crate::text;

const INDEX_USAGE: &str = "Usage: eclipse ai index";
...
every 15 minutes.";
const SEARCH_USAGE: &str = "Usage: eclipse ai search <words>";
...
fn find(words: &str) -> Result<Vec<Hit>, String> {
    ...
    let index = match fs::read(&path) {
        ...
        Err(e) if e.kind() == io::ErrorKind::NotFound => {
            return Err("Nothing is indexed yet. Run eclipse ai index first.".into());
        }
    ...
    if status.embedding_model != index.model {
        return Err(format!(
            "The index was made with {}, and Aura runs {}. Run eclipse ai index to make it again.",
