//! `eclipse net`: the network switch Penumbra keeps for each app that runs in a sandbox. Lists the
//! apps that are off or running, and turns an app's network off or on.

use std::fmt::Write as _;
use std::process::ExitCode;

use libeclipse::penumbra::{self, App};

use crate::text;

const USAGE: &str =
    "Usage: eclipse net [list]\n       eclipse net off <app>\n       eclipse net on <app>";
...
starts later, until on gives it back. An app is named after its command, or by --name when eclipse \
...
pub fn run(args: &[String]) -> ExitCode {
    ...
    match words.as_slice() {
        ...
        [switch @ ("off" | "on")] => {
            eprintln!("eclipse net {switch}: the name of an app is needed\n{USAGE}");
            ExitCode::from(2)
        }
