//! Starts `eclipse-flash write` the way the app does, through the system's prompt for the rights to
...
//!
//! `elevated <eclipse-flash> <image> <disk> <serial>`
...
fn main() -> ExitCode {
    ...
    let [program, image, disk, serial] = args.as_slice() else {
        eprintln!("Usage: elevated <eclipse-flash> <image> <disk> <serial>");
