...
impl Disk {
    /// Whether `rift-flash list` shows it: a whole disk the system does not run from, on USB,
    ...
    pub fn refuse(&self, needed: u64, in_use: Option<&str>) -> Result<(), String> {
        ...
        if self.bus == Bus::Inside {
            return Err(format!(
                "{path} is neither removable nor on USB. Rift is only written onto a stick or a USB disk, so a disk inside a computer is never erased."
            ));
        }
        if self.read_only {
...
mod tests {
    ...
    fn every_rule_has_its_sentence() {
        ...
        refused(
            ...
            "holds 16.0 GiB, and Rift needs 21.1 GiB",
        );
        let mut card = stick();
