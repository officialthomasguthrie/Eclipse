...
fn plist(text: &str) -> Result<Value, String> {
    read_plist(text).map_err(|why| format!("diskutil printed something Rift cannot read. {why}"))
}
...
mod tests {
    ...
    fn a_stick_is_written_even_with_a_volume_mounted() {
        ...
        assert!(stick.listed());
        // macOS mounts every stick, and rift-flash unmounts it before it writes
