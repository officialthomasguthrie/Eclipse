...
//! api. What the model
//! says is either an answer in words or the words of one of Lens's OS commands, which it marks
...
/// What Quasar tells the model before every question.
pub const INSTRUCTIONS: &str = "You are Quasar, the assistant in Rift, a personal operating \
...
/// What Quasar adds when the question is about one of the settings Lens's OS commands change.
/// The commands are the grammar Lens's field reads, and Lens runs nothing from a reply that
...
pub enum Reply {
    ...
    Answer(String),
    /// The words of an OS command, such as `volume 40`. Lens reads them with its own parser.
    Action(String),
}
