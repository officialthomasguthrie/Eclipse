...
const REFUSED: &str = "Aura's local API does not answer web pages.";
...
/// Answers on `listener` for as long as aurad runs, every connection on its own thread.
...
pub fn serve(listener: &TcpListener, socket: &Path, status: &Arc<Mutex<Status>>) {
    ...
    for client in listener.incoming() {
        match client {
            ...
            Err(e) => {
                eprintln!("aurad: the local api could not take a connection: {e}");
...
fn handle(
    ...
    if head.sent_by_a_web_page(port) {
        println!(
            "aura: the local api refused a request from a web page: {}",
            head.line
        );
...
mod tests {
    ...
    fn the_api_answers_programs_and_refuses_pages() {
        ...
        let dir = std::env::temp_dir().join(format!("aura-api-{}", std::process::id()));
