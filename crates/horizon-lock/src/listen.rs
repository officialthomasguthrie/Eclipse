...
fn listen() -> Result<(), String> {
    ...
    // pam_systemd sets the session id and horizon passes it on to what it starts
    ...
    eprintln!("horizon-lock: waiting for logind to lock session {id}");
    ...
    for _ in signals {
        ...
        match Command::new(&program).spawn() {
            ...
            Err(e) => eprintln!("horizon-lock: could not start the lock screen: {e}"),
        }
    }
