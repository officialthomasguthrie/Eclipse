...
//! wayland-sys with dlopen for corona's panel, and features are shared across the workspace. There
//! is no system library path on the image, so the lock screen binary names them as needed
//! libraries, like corona does: the loader finds them through the rpath, and a dlopen by soname
