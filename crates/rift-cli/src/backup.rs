//! `rift backup`: backups of home from the terminal. Lists the backups Vault made on the backup
//! disk, makes one, and restores a file from one the way `rift snapshot restore` does. Choosing
...
const USAGE: &str = "Usage: rift backup [list]\n       rift backup now\n       \
rift backup restore [--replace] <backup> <file>";
