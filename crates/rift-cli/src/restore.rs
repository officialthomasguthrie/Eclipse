//! What `rift snapshot restore` and `rift backup restore` share: the arguments, the full
//! path, and asking before a file that changed is replaced.

use std::process::ExitCode;

use librift::vault::{Refusal, Restored};
...
mod tests {
    ...
    fn each_outcome_is_a_sentence() {
        let (path, snapshot) = ("/home/rift/notes.txt", "2026-09-12T14:00:03Z");
        assert_eq!(
            said(Restored::Restored, path, snapshot),
            "Restored /home/rift/notes.txt from 2026-09-12T14:00:03Z."
        );
        assert_eq!(
            said(Restored::Replaced, path, snapshot),
            "Replaced /home/rift/notes.txt with the copy from 2026-09-12T14:00:03Z."
        );
        assert_eq!(
            said(Restored::Unchanged, path, "backup e863e83c"),
            "/home/rift/notes.txt is the same as in backup e863e83c. Nothing was restored."
        );
    }
}
