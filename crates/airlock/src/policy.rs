...
//! `airlock enter` inside it with the same folders as Landlock rules.
...
mod tests {
    ...
    fn the_bwrap_line() {
        ...
        let line: Vec<String> = policy
            .bwrap(
                Path::new("/nix/store/x-workspace/bin/airlock"),
        ...
        assert!(text.contains(
            "--dev /dev --proc /proc --tmpfs /tmp --tmpfs /home --dir /home/rift \
             ...
             -- /nix/store/x-workspace/bin/airlock enter --read /nix/store"
        ));
    ...
    fn home_is_changeable_when_it_is_the_empty_one() {
        ...
        let text: Vec<String> = policy
            .bwrap(Path::new("/bin/airlock"), &["true".to_string()])
            .iter()
            .map(|word| word.to_string_lossy().into_owned())
            .collect();
        assert!(
