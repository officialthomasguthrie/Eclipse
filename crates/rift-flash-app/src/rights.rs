//! Starting `rift-flash write` with the rights to erase a disk, and reading what it prints while it
...
fn run(job: &Job, events: &Events) -> Result<bool, String> {
    ...
    // closed, so rift-flash never waits for a line it will not get
...
fn run(job: &Job, events: &Events) -> Result<bool, String> {
    ...
    let mut child = Command::new("osascript")
        ...
        .arg(&line)
        .arg(format!(
            "Writing Rift erases {}, and that needs an administrator.",
            job.disk
        ))
        .stdout(Stdio::null())
        .stderr(Stdio::piped())
...
/// What cmd runs as an administrator: rift-flash and its words in double quotes, what it prints
...
/// The two files an elevated rift-flash prints into, in the temporary folder.
fn outputs() -> (PathBuf, PathBuf) {
    let base = std::env::temp_dir().join(format!("rift-flash-{}", std::process::id()));
...
mod tests {
    ...
    fn job(passphrase: Option<&str>) -> Job {
        Job {
            program: "/opt/rift/rift-flash".into(),
            image: "/home/thomas/rift 0.1.0.raw.zst".into(),
            disk: "/dev/sdb".into(),
    ...
    fn the_command_line_gets_the_serial_and_never_the_passphrase() {
        ...
        assert_eq!(
            ...
            [
                ...
                "--serial",
                "4C530001",
                "/home/thomas/rift 0.1.0.raw.zst",
                "/dev/sdb"
            ]
        );
    ...
    fn a_mac_shell_gets_every_word_quoted() {
        let words = [
            "/Applications/rift-flash".to_string(),
            "write".to_string(),
            "/Users/thomas/Thomas's image.raw".to_string(),
        ];
        assert_eq!(
            mac_line(&words, (Path::new("/tmp/f.out"), Path::new("/tmp/f.err"))),
            r"'/Applications/rift-flash' 'write' '/Users/thomas/Thomas'\''s image.raw' > '/tmp/f.out' 2> '/tmp/f.err'"
        );
    }
    ...
    fn cmd_gets_every_word_in_double_quotes() {
        let args = vec![
            "write".to_string(),
            r"C:\Users\thomas\rift.raw.zst".to_string(),
            r"\\.\PhysicalDrive2".to_string(),
        ];
        assert_eq!(
            windows_line(
                Path::new(r"C:\Rift\rift-flash.exe"),
                &args,
                (Path::new(r"C:\Temp\f.out"), Path::new(r"C:\Temp\f.err"))
            )
            .unwrap(),
            r#"/d /s /c ""C:\Rift\rift-flash.exe" "write" "C:\Users\thomas\rift.raw.zst" "\\.\PhysicalDrive2" > "C:\Temp\f.out" 2> "C:\Temp\f.err"""#
        );
        let percent = vec![r"C:\100%\rift.raw".to_string()];
        assert!(
            windows_line(
                Path::new("rift-flash.exe"),
                &percent,
    ...
    fn a_growing_file_is_read_a_line_at_a_time() {
        let path =
            std::env::temp_dir().join(format!("rift-flash-app-lines-{}", std::process::id()));
