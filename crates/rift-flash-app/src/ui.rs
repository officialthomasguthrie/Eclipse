//! The window: the image, the disks rift-flash lists, the passphrase on Linux and the serial or name
//! typed back, then the steps rift-flash prints while it writes. Drawn with iced in software in a
...
use librift::disk::passphrase_problem;
use rift_flash::Listed;
...
struct Flash {
    ...
    passphrase: String,
    again: String,
    /// rift-flash, or why it is not found.
...
/// A write that was started, and the lines rift-flash printed so far.
struct Writing {
    disk: String,
    lines: Vec<Line>,
    ended: Option<bool>,
...
/// rift-flash in the folder the app is in, or on the path.
...
/// The image at `path`, which is made absolute: rift-flash starts with rights in a folder of its
/// own, and the page says which folder the image is in.
fn chosen(path: &Path) -> Chosen {
    let path = std::path::absolute(path).unwrap_or_else(|_| path.to_path_buf());
    Chosen {
        version: rift_flash::version(&path),
        path,
    }
}
...
fn read_disks() -> Task<Message> {
    ...
    std::thread::spawn(move || {
        let _ = sender.send(rift_flash::disks());
    });
    ...
    match message {
        Message::Choose => Task::perform(
            rfd::AsyncFileDialog::new()
                .set_title("Choose a Rift image")
                .add_filter("Rift image", &["raw", "zst"])
                .pick_file(),
...
/// Starts rift-flash with the rights to write, and forgets the passphrase and what was typed.
...
/// A disk as rift-flash lists it: what it is, and why it is refused when it is. One that passes can
...
/// The page that shows what rift-flash prints while it writes, and the sentence it ends on.
fn steps(writing: &Writing, colors: Colors) -> Element<'_, Message> {
    let title = match writing.ended {
        None => format!("Writing Rift onto {}", writing.disk),
        Some(true) => format!("Rift was written onto {}", writing.disk),
        Some(false) => format!("Rift was not written onto {}", writing.disk),
    };
