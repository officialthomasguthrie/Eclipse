//! What Windows says about its disks: the answer to the PowerShell query eclipse-flash runs, Get-Disk
...
pub fn read_get_disk(json: &str) -> Result<(bool, Vec<Disk>), String> {
    let answer: Answer = serde_json::from_str(json.trim_start_matches('\u{feff}'))
        .map_err(|e| format!("PowerShell printed something Eclipse cannot read: {e}"))?;
...
mod tests {
    ...
    fn a_stick_is_written_even_with_a_drive_letter() {
        ...
        assert!(stick.listed());
        // Windows gives its sticks a letter, and eclipse-flash clears the disk before it writes
