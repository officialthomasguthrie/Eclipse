...
pub const HEADER: &[u8] = b"eclipse search index 1\n";
...
/// Where the index of a home is kept: in `eclipse` under the owner's cache folder, which is
...
mod tests {
    ...
    impl Home {
        fn new(name: &str) -> Self {
            let path =
                std::env::temp_dir().join(format!("eclipse-search-{name}-{}", std::process::id()));
