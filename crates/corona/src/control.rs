...
//! directory; `corona --type`, `corona --enter` and `corona --escape` write one line to it.
...
pub fn send(command: &Command) -> Result<(), String> {
    let path = path().ok_or("Corona could not find the session runtime directory")?;
