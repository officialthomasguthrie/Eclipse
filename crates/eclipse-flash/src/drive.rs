//! What eclipse-flash checks without touching a disk: sizes typed in, the folder of models, whether
...
#[cfg(test)]
mod tests {
    use libeclipse::disk::{Partition, read_table};
    ...
    fn a_folder_of_models_is_its_files() {
        let folder =
            std::env::temp_dir().join(format!("eclipse-flash-models-{}", std::process::id()));
    ...
    /// What sfdisk prints for a file eclipse-flash wrote a table into, with an exchange partition.
    const WRITTEN: &str = r#"{
       "partitiontable": {
