...
/// Runs eclipse-flash with `args`, the words after its name, and returns its exit code.
#[must_use]
...
/// The sticks and USB disks `eclipse-flash list` shows, each with the sentence that says why a drive
...
/// The version of Eclipse in the image at `path`, whose partition table is read and checked the way
/// `write` reads it.
///
/// # Errors
///
/// The sentence that says why the file is not an image eclipse-flash can write.
pub fn version(path: &Path) -> Result<String, String> {
    image::Image::open(path).map(|image| image.slot.version)
}

/// What `eclipse-flash list` prints: each disk, why it is refused when it is, and a blank line
/// between two.
...
#[cfg(not(any(target_os = "linux", target_os = "macos", windows)))]
const UNSUPPORTED: &str = "eclipse-flash cannot write a drive on this system.";
