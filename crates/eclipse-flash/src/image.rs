//! The system image eclipse-flash writes: its partition table and the slot in it, and its bytes from
...
impl Image {
    ...
    pub fn open(path: &Path) -> Result<Image, String> {
        ...
        let refused = |why: String| format!("{shown} is not an Eclipse image. {why}");
...
#[cfg(test)]
mod tests {
    use libeclipse::disk::{MIB, read_table};
    ...
    fn a_file_that_is_not_an_image_is_refused_when_it_is_opened() {
        ...
        assert!(
            why.ends_with("empty.raw is not an Eclipse image. It has no GUID partition table."),
            "{why}"
        );
